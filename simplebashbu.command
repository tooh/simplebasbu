...
# - FILESYSTEM can contain space in name. Could not make this work (variables need enclosing by " etc). Temporarily changed folders names from source
# - LOG file name datestamp changed to %d%m%Y
# - call to bu_notify.py to send message (independent from sendmail etc)

# without this OSX will copy the _. resource files.
...
# Only keep last weeks level0 backup (0) or keep all lvl 0 backups (1).  Keeping all data may take a lot of space!
  KEEPALL=0
# Do we want to compress the backup file using gzip? 0=false, 1=true
  COMPRESS=1
...
print "$1 TIME:$TIME" | tee -a $LOGFILE # if rc > 0 then print error msg and quit
# Mail when backup error
python ~/utils/bu_error.py
exit $?
fi
}
...
# Logfile
  LOGFILE=$LOGDIR/`date +"%d%m%Y_%s"`.log
...
# Does the daily backup log dir exist? If not, create it.
if [ ! -d $LOGDIR ]
  then
    mkdir $LOGDIR
fi
...
if test $DAYOFWEEK -eq $LEVEL0DAY
  then
    ...
    do
      ...
      OUTFILENAME=$WITHOUTLEADINGDOT.`date +"%d%m%Y_%s"`.tar
    ...
    if [ ! -w $L0DATESTAMP ]
      then
       touch $L0DATESTAMP
       echo "1969-12-31 12:00:00" > $L0DATESTAMP
    fi
    ...
    do
      ...
      echo "#######################################################################"
      echo "$STARTTIME: Creating a level $LEVEL backup of $BACKUPFILES to $OUTFILE" 
      echo "Last rundate was : $LAST Now is : $NOW "
      echo "#######################################################################"
      tar --create $VERBOSECOMMAND \
        --file "$OUTFILE" \
        --newer "$LAST" \
...
#Fix stdout and stderr to go to console instead of file since we are 
#about to compress the log file
exec 1>&3                   
exec 2>&3

cp $LOGFILE $LOGDIR/latest.log
...
# Mail when backup is ready
python ~/utils/bu_notify.py

# Speech message
say -v Claire "Backup is klaar"
